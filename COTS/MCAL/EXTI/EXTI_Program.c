    ...
    switch ( Copy_u8INTnumber )
    {
        case INT0 :     SET_BIT(GICR_REG,PIN6);             // Set 1 On PIN6 At GTCR_REG to [Enable] (PIE) Peripheral Interrupt Enable to INT0 .
                        ...
                        {                                                                                                   // BIT1  -  BIT0  on MCUCR_REG
                            case FALLING_EDGE :     SET_BIT(MCUCR_REG,PIN1);      CLR_BIT(MCUCR_REG,PIN0);        break;    //   1   -   0   Falling Edge
                            case RAISING_EDGE :     SET_BIT(MCUCR_REG,PIN1);      SET_BIT(MCUCR_REG,PIN0);        break;    //   1   -   1   Raising Edge
                            case LOW_LEVEL    :     CLR_BIT(MCUCR_REG,PIN1);      CLR_BIT(MCUCR_REG,PIN0);        break;    //   0   -   0   Low Level
                            case ON_CHANGE    :     CLR_BIT(MCUCR_REG,PIN1);      SET_BIT(MCUCR_REG,PIN0);        break;    //   0   -   1   On Change
                            default           :                                                                   break;
                        }
                        break;
        case INT1 :     SET_BIT(GICR_REG,PIN7);             // Set 1 On PIN7 At GTCR_REG to [Enable] (PIE) Peripheral Interrupt Enable to INT1 .
                        ...
                        {                                                                                                   // BIT3  -  BIT2  on MCUCR_REG
                            case FALLING_EDGE :     SET_BIT(MCUCR_REG,PIN3);      CLR_BIT(MCUCR_REG,PIN2);        break;    //   1   -   0   Falling Edge
                            case RAISING_EDGE :     SET_BIT(MCUCR_REG,PIN3);      SET_BIT(MCUCR_REG,PIN2);        break;    //   1   -   1   Raising Edge
                            case LOW_LEVEL    :     CLR_BIT(MCUCR_REG,PIN3);      CLR_BIT(MCUCR_REG,PIN2);        break;    //   0   -   0   Low Level
                            case ON_CHANGE    :     CLR_BIT(MCUCR_REG,PIN3);      SET_BIT(MCUCR_REG,PIN2);        break;    //   0   -   1   On Change
                            default           :                                                                   break;
                        }
                        break;
        case INT2 :     SET_BIT(GICR_REG,PIN5);             // Set 1 On PIN5 At GTCR_REG to [Enable] (PIE) Peripheral Interrupt Enable to INT2 .
                        switch ( Copy_u8SenseControl )
                        {                                                                      //  BIT6  on  MCUCSR_REG
                            case FALLING_EDGE :     CLR_BIT(MCUCSR_REG,PIN6);      break;    //   0    Falling Edge
...
void EXTI_voidDisable ( u8 Copy_u8INTnumber )
{
    switch ( Copy_u8INTnumber )
    {
        case INT0 :     CLR_BIT(GICR_REG,PIN6);   break;    // Set 0 On PIN6 At GTCR_REG to [Disable] (PIE) Peripheral Interrupt Enable to INT0 .
        case INT1 :     CLR_BIT(GICR_REG,PIN7);   break;    // Set 0 On PIN7 At GTCR_REG to [Disable] (PIE) Peripheral Interrupt Enable to INT1 .
        case INT2 :     CLR_BIT(GICR_REG,PIN5);   break;    // Set 0 On PIN5 At GTCR_REG to [Disable] (PIE) Peripheral Interrupt Enable to INT2 .
        default   :                                 break;
    }
}
